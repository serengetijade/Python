...
def createDB(self):                                         #Create a database, access the "self" object
    conn = sqlite3.connect('db_studentTracking.db')               #If the database doesn't exist, the command will create it. Otherwise, it will connect to the named database, and the variable "conn" will represent that connection.
    with conn:                                              #Signal the program to receive instructions relating to the (connected) database.
        cur=conn.cursor()                                   #cursor() is a python method to influence the (connected) database.
        cur.execute("CREATE TABLE IF NOT EXISTS table_studentTracking( \
...
def firstRun(self):                                         #(self) connects this function to the class named "self"
    conn = sqlite3.connect('db_studentTracking.db')               #If the named database exists, the sqlite3.connect() method creates a connection to it.
    with conn:
        cur = conn.cursor()                                 #cursor() is a python method to influence the (connected) database.
        ...
        if count < 1:                                       #Ensure there is always one entry, to avoid errors. 
            cur.execute("""INSERT INTO table_studentTracking (First_Name, Last_Name, Full_Name, Phone, Email, Current_Courses) \
            VALUES (?,?,?,?,?,?)""", ('Norma', 'Jean', 'Norma Jean', '503-456-1234', 'callmemarilyn@gmail.com', 'Mathematics'))       #Use wildcards to represent future values. Add an example entry to avoid errors. 
            conn.commit()
    conn.close()                                            
...
def countRecords(cur):                                      #You must pass in (cur) so that when the function is called in line 54, it can also call (cur) there. And because cur is defined in that function, here it will pass those values into the rest of this function.
    ...
    cur.execute("""SELECT COUNT(*) FROM table_studentTracking""") #use .execute() method to signal the program to recieve SQL commands from the "cur" object. Use SQL commands to count the records(rows) in the connected table.  
...
def onSelect(self,event):                                   #Name the function, then pass in the (self, so it knows the name of the class to affect, and pass in ,event) to tell python this is an event.
    ...
    conn = sqlite3.connect('db_studentTracking.db')               #Create a variable to represent the connectoin to the database
    with conn:                                              
        ...
        cursor.execute("""SELECT First_Name, Last_Name, Phone, Email, Current_Courses FROM table_studentTracking\
        ...
        for data in varBody:                                #Loop to access each indexed object in the tuple
            self.entryFN.delete(0,END)                      #Clear the textbox
            self.entryFN.insert(0,data[0])                  #Insert the value at index [0] and so on...
            ...
            self.entryCurrentCourses.delete(0,END)
            self.entryCurrentCourses.insert(0,data[4])
...
def addToList(self):                                        #Function to add to the list within the self class object.
    ...
    var_courses = self.entryCurrentCourses.get().strip()
    if not "@" or not "." in var_email:                     #Ensure a valid email address is entered.
        print("Incorrect email format. Please retry.")
    if (len(var_fname)>0) and (len(var_lname)>0) and (len(var_phone)>0) and (len(var_email)>0 and (len(var_courses)>0)): #Force the user to fill out all fields.
        conn = sqlite3.connect('db_studentTracking.db')
        with conn:
            cursor = conn.cursor()
            cursor.execute("""SELECT COUNT(Full_Name) FROM table_studentTracking \
            ...
            if checkName == 0:                              #If the full name does NOT already exist in the table, do this:
                ...
                cursor.execute("""INSERT INTO table_studentTracking (First_Name, Last_Name, Full_Name, Phone, Email, Current_Courses) \
                    VALUES (?,?,?,?,?,?)""",(var_fname, var_lname, var_fullname, var_phone, var_email, var_courses)) #Check the database for existance of the fullname, if so we will alert user and disregard request. When count != 0, the name is already in use.
...
def onDelete(self):
    ...
    with conn:                                              #Create a variable to represent the cursor() method. cursor() is a python method to influence the (connected) database.
        cur = conn.cursor()
        cur.execute("""SELECT COUNT(*) FROM table_studentTracking""")     #Use .execute() to enter a SQL command to count all rows in the table.
        ...
        if count > 1:                                       #So long as the count is GREATER than 1, the database is NOT empty.
            ...
            if confirm:                                     #This treats confirm as a boolean value, so as long as it is true, do the following code:
                conn = sqlite3.connect('db_studentTracking.db')   #Connect to the database
                with conn:                                  #While the connection is open:
                    cursor = conn.cursor()                  #Resent the cursor variable
                    cursor.execute("""DELETE FROM table_studentTracking WHERE Full_Name = '{}'""".format(var_select)) #Use the exectute() method to perform SQL commands.
...
def onRefresh(self):                                        #Populate the listbox within the self class, coinciding with the database
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT COUNT(*) FROM table_studentTracking""")  #Use the exectute() method to perform SQL commands.
        ...
        while i < count:                                    #Create an iteration loop that runs as long as i is less than the value of the count variable (the number of objects from table_phonebook)
            cursor.execute("""SELECT Full_Name FROM table_studentTracking""")    #Select the Full Name for every row
...
def onUpdate(self):                                         #Function to allow updates to the database, and therefore the listbox.
    ...
    var_email = self.entryEmail.get().strip()
    var_courses = self.entryCurrentCourses.get().strip()
    var_fullname = self.entryFN.get().strip() +' '+ self.entryLN.get().strip()
    if (len(var_phone)>0) and (len(var_email)>0):           #Enforce that the phone and email are NOT empty in order to access the database.
        conn = sqlite3.connect('db_studentTracking.db')
        with conn:
            cur = conn.cursor()
            cur.execute("""SELECT COUNT(Phone) FROM table_studentTracking WHERE Phone='{}'""".format(var_phone))  #Ensure the provided Phone is already in the database.
            ...
            print(count1)
            cur.execute("""SELECT COUNT(Email) FROM table_studentTracking WHERE Email='{}'""".format(var_email))  #Ensure the provided Email is already in the database.
            count2 = cur.fetchone()[0]                      #Python method .fetchone() extracts the value from the cur variable, i.e the 'Email'
            print(count2)
            cur.execute("""SELECT COUNT(Current_Courses) FROM table_studentTracking WHERE Current_Courses='{}'""".format(var_courses))  #Ensure the provided Email is already in the database.
            count3 = cur.fetchone()[0]                      #Python method .fetchone() extracts the value from the cur variable, i.e the 'Email'
            print(count3)
            if count1 == 0 or count2 == 0 or count3 ==0:                  #Count records to see if the user's changes are already in the database. If they are already there, there are no changes to update. If one return value is 0, that means there was no match and there IS something that can be updated.
                response = messagebox.askokcancel("Update Request","The following changes ({}), ({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_phone, var_email, var_courses, var_value, ))
                print(response)
                if response:                                #This treats 'response' as a boolean value, so as long as it is true, do the following code:
                    ...
                    with conn:
                        ...
                        cursor.execute("""UPDATE table_studentTracking SET Phone = '{0}', Email = '{1}', Current_Courses = '{2}' WHERE Full_Name = '{3}'""".format(var_phone, var_email, var_courses, var_fullname)) #Use .execute() method to enter SQL commands.
                ...
                messagebox.showinfo("No changes detected", "Both ({}), ({}) and ({})\nalready exist in the database for this name.\n\nYour update request has been cancelled.".format(var_phone, var_email, var_courses)) #If the name and email return 1, they must already exist. So therefore they cannot be updated to their own value.
            onClear(self)                                   #Call the onClear() function and tell it to affect the class 'self'
        conn.close()
    else:
